...
const TreeNode = memo(({ 
  ...
  const active = selectedFileId === item.id;
...
const FileExplorer = ({ 
  ...
  const toggleFolder = React.useCallback((folderId) => {
  ...
  const fileTree = useMemo(() => {
    ...
    // First pass: Create lookup for folders
    files.forEach(file => {
      if (file.isFolder) {
    ...
    files.forEach(file => {
      ...
      const parent = lookup[parentId] || tree;
  ...
  return (
    <div className="w-64 h-full bg-[#09090b] flex flex-col shrink-0 overflow-hidden border-r border-white/[0.06]">
